...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $pdo->prepare("INSERT INTO trips (user_id, title, destination, location, start_date, end_date, notes, expenses, image, visibility) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            if ($stmt->execute([$_SESSION['user_id'], $title, $destination, $location, $start_date, $end_date, $notes, $expenses, $image, $visibility])) {
                ...
                $pdo->rollBack();
                $errors[] = "Failed to add trip.";
            }
        } catch (Exception $e) {
            $pdo->rollBack();
            $errors[] = "Failed to add trip: " . $e->getMessage();
        }
    }
}
?>
...
<head>
    ...
    <title>Add Trip - Travel Tracker</title>
...
<body class="bg-cover bg-center bg-fixed h-screen" style="background-image: url('images/travel8.jpg'); font-family: 'Inter', Arial, sans-serif;">
    <div class="flex flex-col min-h-screen bg-black bg-opacity-50">
        ...
        <div class="flex flex-1 items-center justify-center mb-20">
            <div class="bg-white bg-opacity-60 rounded-lg shadow-lg p-8 w-full max-w-xl my-12">
                <h2 class="text-2xl font-bold mb-6 text-center text-blue-900">Add New Trip</h2>
                <?php if (!empty($errors)): ?>
                ...
                <form method="POST" class="space-y-4 max-w-lg mx-auto" enctype="multipart/form-data">
                    <div>
                        ...
                        <input type="text" name="title" id="title" required minlength="3" class="w-full p-2 border rounded">
                    </div>
                    <div>
                        <label for="destination" class="block text-sm font-medium mb-2">Destination</label>
                        <input type="text" name="destination" id="destination" class="w-full p-2 border rounded">
                    </div>
                    <div>
                        <label for="location" class="block text-sm font-medium mb-2">Location</label>
                        <input type="text" name="location" id="location" class="w-full p-2 border rounded" placeholder="e.g. Paris, France">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="number" name="expenses" id="expenses" step="0.01" min="0" class="w-full p-2 border rounded">
                    </div>
                    <div>
                    ...
                    <div>
                        <label for="additional_images" class="block text-sm font-medium mb-2">Additional Images</label>
                    ...
                    <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Add Trip</button>
                </form>
            </div>
        </div>
    </div>
